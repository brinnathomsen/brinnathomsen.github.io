...
//   var scrollDown = window.setInterval(scrolling, 500);
//
// });
...
function repeatText() {
  ...
  $(".stage" + stages).css("transform", "scale(.15)");
  $(".stage" + stages).css("position", "absolute");
  $(".stage" + stages).css("top", "0px");
  $(".stage" + stages).css("background", "red");
  $(".stage" + stages).css("background", "red");
  $(".stage" + stages).css("width", "1000px");


  multiplier++;
  newPosY = multiplier -1;
  ...
  $(".stage" + stages).css("top", newPosY + "px");
  $(".stage" + stages).css("left", newPosX + "px");
...
$(window).keypress(function(event) {
  ...
  if (event.which == 32) {
    ...
    $(".typing .text").append(newBigLetter);
    newWord = $(".alphabet .space").clone();
    $(".display").append(newBigLetter);


    // start new word in 'history' text
    newWord = $(".display").append(
      "<div class='happy wor" + wordNumber + "'></div>");
    newWord = $(".happy");

     $(".wor" + wordNumber).css("width", "500px");
    $(newWord).append(newLetter);
  }
});
